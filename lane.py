...
class Lane(object):
    ...
    def draw(self, img, verbose=0):
        ...
        result, M, Minv = warper(result, np.float32(src), np.float32(dst))

        # step 5: fit poly
        left_fit, right_fit, left_curverad, right_curverad, dist_center_px, img_poly = self.fit_polynomial(result, verbose=verbose)
        if verbose:
            plt.imshow(img_poly)
            plt.show()
        ...
        img_overlay2 = cv2.resize(cv2.cvtColor(255 * img_binary, cv2.COLOR_GRAY2RGB), (256, 144))
